...
Feature: UI user lib
    ...
    Scenario: Ajout d'un utilisateur
        ...
        When click("{^}Créer un utilisateur")
            And input("#editInfoUserNameInput", [username, Key.ENTER], 100)
            And input("#editInfoLastNameInput", [lastName, Key.ENTER], 100)
            And input(ip5.ui.locator.input("Prénom"), [firstName, Key.ENTER], 100)
            And input(ip5.ui.locator.input("E-mail"), [email, Key.ENTER], 100)
            And input(ip5.ui.locator.input("Nouveau mot de passe"), [password, Key.ENTER], 100)
            And input(ip5.ui.locator.input("Confirmer le mot de passe"), [password, Key.ENTER], 100)
