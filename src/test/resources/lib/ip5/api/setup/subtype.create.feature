...
Feature: Subtype setup lib
  ...
  Scenario: Create subtype
    ...
    * def prepareSubtypeLayerList =
    """
    (tenantId, subtypeLayerList) => {
        let result = [];
        for (let i = 0;i < subtypeLayerList.length;i++) {
            for (const [layer, association] of Object.entries(subtypeLayerList[i])) {
                let layerId = ip5.api.v1.layer.getIdByName(tenantId, layer);
                result.push({
                    "layerId": layerId,
    ...
    * def cleanupPayload =
    """
    (payload, defaults) => {
        let keys = ['externalSignatureConfig', 'multiDocuments', 'secureMailServerId', 'sealCertificateId', 'subtypeLayerRequestList', 'subtypeMetadataList', 'workflowSelectionScript'];
        for (let key of keys) {
