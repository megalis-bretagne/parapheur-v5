...
"""
        ...
        * def rv = call read('classpath:lib/soap/requests/RechercherDossiers/simple.feature') __arg
        * def dossiersIds = karate.xmlPath(rv.response, '/Envelope/Body/RechercherDossiersResponse/LogDossier/nom');
        # "Filtrage" des résultats
        * def dossierId = api.soap.dossier.filterDossiersIdsByName(dossiersIds, __arg.name, { username: "ws@legacy-bridge", password: "a123456" })
        ...
        * call read('classpath:lib/soap/requests/GetHistoDossier/simple.feature') params
        * match response == karate.read('classpath:lib/soap/schemas/GetHistoDossierResponse/OK.xml')
