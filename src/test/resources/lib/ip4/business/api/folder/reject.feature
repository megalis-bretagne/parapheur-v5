...
Feature: IP v.4 REST folder lib
    ...
    Scenario: Reject folder
        # 1. Préparation
        * def publicAnnotation = ip.templates.annotations.getPublic(__arg.username, "rejet", __arg.folder)
        * def privateAnnotation = ip.templates.annotations.getPrivate(__arg.username, "rejet", __arg.folder)
