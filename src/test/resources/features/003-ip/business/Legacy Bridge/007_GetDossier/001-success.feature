...
Feature: GetDossier
    ...
    Scenario Outline: Récupération du dossier en fin de circuit (type "${type}", sous-type "${sousType}", status "${status}")
          ...
          Given def params = karate.merge(__row, { schema: "OK.xml", username: "ws@legacy-bridge", password: "a123456a123456" })
          ...
          Given def params = karate.merge(__row, { dossierId: dossierId, username: "ws@legacy-bridge", password: "a123456a123456" })
