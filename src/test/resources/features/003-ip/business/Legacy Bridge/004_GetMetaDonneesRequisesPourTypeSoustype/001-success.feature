...
Feature: GetMetaDonneesRequisesPourTypeSoustype
    ...
    Scenario Outline: Récupération des méta-données du sous-type "${type}" / "${sousType}"
        ...
        When def rv = call read('classpath:lib/ip/api/soap/requests/GetMetaDonneesRequisesPourTypeSoustype/simple.feature') params
        Then xmlstring actual = karate.xmlPath(rv.response, '/Envelope/Body/GetMetaDonneesRequisesPourTypeSoustypeResponse/MetaDonnee')
            ...
            And match rv.response == karate.read('classpath:lib/ip/api/soap/schemas/GetMetaDonneesRequisesPourTypeSoustypeResponse/OK.xml')
    ...
    Scenario Outline: Récupération des méta-données du sous-type "${type}" / "${sousType}"
        ...
        Then match karate.xmlPath(rv.response, 'count(/Envelope/Body/GetMetaDonneesRequisesPourTypeSoustypeResponse/MetaDonnee)') == 0
            And match rv.response == karate.read('classpath:lib/ip/api/soap/schemas/GetMetaDonneesRequisesPourTypeSoustypeResponse/OK-empty.xml')
