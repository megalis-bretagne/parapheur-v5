...
Feature: ACTES-CAdES - Signature - PDF_sans_tags - signe_cades

    Background:
        * ip.pause(2)
        * def type = "ACTES - CAdES"
        * def subtype = "Signature"
        * def name = "ACTES-CAdES - Signature - PDF_sans_tags - signe_cades"
        * def files = [ { file: "classpath:files/formats/PDF_sans_tags/PDF_sans_tags.pdf", detached: "classpath:files/formats/PDF_sans_tags/signature_cades.p7s" } ]
    ...
    Scenario Outline: Vérifications de la liste des fichiers (${key})
        ...
        * match download.files == [ "PDF_sans_tags.pdf", "PDF_sans_tags.pdf-0-signature_externe.p7s", "PDF_sans_tags.pdf-1-<user>.p7s" ]
    ...
    Scenario Outline: Vérifications des fichiers non signés (${key})
        ...
        * match karate.read("file://" + download.base + "/PDF_sans_tags.pdf-0-signature_externe.p7s") == ip.commonpath.read("PDF_sans_tags/signature_cades.p7s")
