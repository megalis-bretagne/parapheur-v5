...
Feature: ACTES-CAdES - Signature - PDF_avec_tags - signe_cades

    Background:
        * ip.pause(2)
        * def type = "ACTES - CAdES"
        * def subtype = "Signature"
        * def name = "ACTES-CAdES - Signature - PDF_avec_tags - signe_cades"
    ...
    Scenario Outline: Vérifications de la liste des fichiers (${key})
        ...
        * match download.files == [ "PDF_avec_tags.pdf", "PDF_avec_tags.pdf-0-signature_externe.p7s", "PDF_avec_tags.pdf-1-<user>.p7s" ]
