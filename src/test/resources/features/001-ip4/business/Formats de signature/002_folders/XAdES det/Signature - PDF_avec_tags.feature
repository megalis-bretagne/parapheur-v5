...
Feature: XAdES det - Signature - PDF_avec_tags
    ...
    Scenario Outline: Vérifications des signatures détachées (${key})
        ...
        * ip4.signature.xades.validate(download.base + "/PDF_avec_tags.pdf", download.base + "/PDF_avec_tags-1-<user>.xml")

        * def expected = {"City":"#notpresent","PostalCode":"#notpresent","CountryName":"#notpresent","ClaimedRole":"#notpresent"}
        * match ip4.signature.xades.extract(download.base + "/PDF_avec_tags-1-<user>.xml") == expected
