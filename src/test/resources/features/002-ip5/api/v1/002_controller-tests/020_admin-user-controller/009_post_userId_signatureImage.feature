...
Feature: POST /api/provisioning/v1/admin/tenant/{tenantId}/user/{userId}/signatureImage (Create user's signature image)
    ...
    Scenario Outline: ${ip5.scenario.title.permissions(role, 'create a signature image for an existing user in an existing tenant', status)}
        ...
        * ip5.api.v1.auth.login('<username>', '<password>')
        ...
        Examples:
            ...
            | ADMIN            | cnoir        | Ilenfautpeupouretreheureux  |                        | classpath:files/images/signature - stranslucide.png | 201    |
            | TENANT_ADMIN     | vgris        | Ilenfautpeupouretreheureux  |                        | classpath:files/images/signature - stranslucide.png | 201    |
            | FUNCTIONAL_ADMIN | ablanc       | Ilenfautpeupouretreheureux  | stranslucide@dom.local | classpath:files/images/signature - stranslucide.png | 403    |
            | NONE             | ltransparent | Ilenfautpeupouretreheureux  | stranslucide@dom.local | classpath:files/images/signature - stranslucide.png | 403    |
