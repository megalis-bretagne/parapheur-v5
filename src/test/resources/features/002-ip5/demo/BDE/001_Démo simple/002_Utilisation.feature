...
Feature: 002 - Scénario de démo simple, partie utilisation
    ...
    Scenario Outline: Envoi de ${count} dossier(s) "${nameTemplate}" dans le circuit pour le sous-type ${type} / ${subtype}
        ...
        * def result = call read('classpath:lib/ip5/api/draft/create-and-send-monodoc-without-annex.feature') folders
        ...
        Examples:
            ...
            | Démo simple | ws@demo-simple | Ilenfautpeupouretreheureux  | WebService | classpath:files/formats/document_office/document_office.doc             | ACTES | Visa    | Délibération DOC %counter%            | 2      | démarrage  |
            | Démo simple | ws@demo-simple | Ilenfautpeupouretreheureux  | WebService | classpath:files/formats/document_libre_office/document_libre_office.odt | ACTES | Visa    | Délibération ODT %counter%            | 2      | démarrage  |
            | Démo simple | ws@demo-simple | Ilenfautpeupouretreheureux  | WebService | classpath:files/formats/PDF_avec_tags/PDF_avec_tags.pdf                 | ACTES | Visa    | Délibération PDF %counter%            | 2      | démarrage  |
            | Démo simple | ws@demo-simple | Ilenfautpeupouretreheureux  | WebService | classpath:files/formats/document_rtf/document_rtf.rtf                   | ACTES | Visa    | Délibération RTF %counter%            | 2      | démarrage  |
            | Démo simple | ws@demo-simple | Ilenfautpeupouretreheureux  | WebService | classpath:files/formats/PDF_avec_tags/PDF_avec_tags.pdf                 | ACTES | Visa    | Demande avis complémentaire %counter% | 2      | démarrage  |

    Scenario Outline: ${action} sur le dossier "${name}" (ACTES/Visa)
        * ip5.ui.user.login("flosserand@demo-simple", "Ilenfautpeupouretreheureux")
    ...
    Scenario Outline: Demande d'avis complémentaire et ${action} sur le dossier "${name}" (ACTES/Visa)
        ...
        * ip5.ui.user.login("mpiaumier@demo-simple", "Ilenfautpeupouretreheureux")
    ...
    Scenario Outline: Vérifications des annotations du dossier ${title} "${name}" (ACTES/Visa)
        * ip5.ui.user.login("ws@demo-simple", "Ilenfautpeupouretreheureux")
