...
"""
    ...
    Scenario Outline: Vérifications des impressions (avec le bordereau de signature) du dossier ${title} "${name}" (ACTES/Visa)
        ...
        * mouse().move("{^button}Actions").go()
        * click("{^button}Actions")
        * waitFor("{^}Générer le bordereau d'impression").click()
        * waitFor("//button[contains(normalize-space(.),'Générer le bordereau d\'impression')]").click()
        * waitFor("{^}Annuler").click()
        * waitForResultCount("//button[contains(normalize-space(.),'Générer le bordereau d\'impression')]", 0)
