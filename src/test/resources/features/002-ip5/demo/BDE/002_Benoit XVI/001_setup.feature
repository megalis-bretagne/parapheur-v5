...
Feature: Paramétrage métier "Benoit XVI"
    ...
    Scenario Outline: Associate user "${email}" with tenant "${tenant}"
        * call read('classpath:lib/ip5/api/setup/tenant.user.associate.feature') __row
        ...
        Examples:
            | email                 | tenant     |
            | sample-user@dom.local | Benoit XVI |
    ...
    Scenario Outline: Create a seal certificate from file "${path}" in "${tenant}"
        * call read('classpath:lib/ip5/api/setup/seal-certificate.create.feature') __row
        ...
        Examples:
            ...
            | Benoit XVI | classpath:files/Entité initiale - Seal Certificate.p12 | christian.buffin@libriciel.coop | 'classpath:files/images/cachet - benoit xvi.png' |
    ...
    Scenario Outline: Create desk "${name}" in "${tenant}"
        * call read('classpath:lib/ip5/api/setup/desk.create.feature') __row
        ...
        Examples:
            ...
            | Benoit XVI | APPLICATIONS                          | APPLICATIONS            | ['sample-user@dom.local']                                                                                                               | ''                                   | []                                                                                                | {}                                                                      |
            | Benoit XVI | Application Générique                 | Application Générique   | ['benoit.demortain+admin50+benoit-xvi@libriciel.coop', 'benoit.demortain+generique+benoit-xvi@libriciel.coop', 'sample-user@dom.local'] | 'APPLICATIONS'                       | []                                                                                                | {'action': true, 'archiving': true, 'chain': true, 'creation': true}    |
            ...
            | Benoit XVI | Président du département              | President Departement   | ['benoit.demortain+tarpex+benoit-xvi@libriciel.coop', 'sample-user@dom.local']                                                          | ''                                   | []                                                                                                | {'action': true, 'archiving': true, 'chain': true, 'creation': false}   |
            ...
            | Benoit XVI | Directeur des Finances                | Directeur Finances      | ['benoit.demortain+admin50+benoit-xvi@libriciel.coop', 'benoit.demortain+morbier+benoit-xvi@libriciel.coop', 'sample-user@dom.local']   | 'Directrice Générale des Services'   | []                                                                                                | {'action': true, 'archiving': false, 'chain': false, 'creation': false} |
    ...
    Scenario Outline: Create subtype "${name}" for type "${type}" and "${validationWorkflowId}" workflow in "${tenant}"
        * call read('classpath:lib/ip5/api/setup/subtype.create.feature') __row
        ...
        Examples:
            ...
            | Benoit XVI | COURRIERS          | Courrier à cacheter         | true                | false           | ['Application Générique', 'Service Finances', 'Service Informatique', 'Service des Marchés Publics', 'Service des Ressources Humaines'] |                    | CS_Pres                         |                           | true           | Christian Buffin - Entité initiale - Cachet serveur |                    | false                      |                                                                     | []                                        |
            | Benoit XVI | COURRIERS          | Courrier à signer Président | true                | false           | ['Application Générique', 'Service Finances', 'Service Informatique', 'Service des Marchés Publics', 'Service des Ressources Humaines'] | V_Chefde           | S_CS_Pres                       |                           | true           | Christian Buffin - Entité initiale - Cachet serveur |                    | true                       |                                                                     | []                                        |
