    ...
    Scenario Outline: Create a seal certificate from file "${path}" in "${tenant}"
        * call read('classpath:lib/ip5/api/setup/seal-certificate.create.feature') __row
        ...
        Examples:
            ...
            | Legacy Bridge | classpath:files/Entité initiale - Seal Certificate.p12 | christian.buffin@libriciel.coop | 'classpath:files/images/cachet - benoit xvi.png' |
    ...
    Scenario Outline: Create subtype "${name}" for type "${type}" and "${validationWorkflowId}" workflow in "${tenant}"
        * call read('classpath:lib/ip5/api/setup/subtype.create.feature') __row
        ...
        Examples:
            ...
            | Legacy Bridge | PAdES         | cachet         | false               | false           | ['WebService']            |                    | Cachet               |                           | null           | Christian Buffin - Entité initiale - Cachet serveur |                    | true                       |                          | []                | []                                                                                           | false           |
            | Legacy Bridge | PAdES         | mailsec        | false               | false           | ['WebService']            |                    | Mailsec              |                           | null           | Christian Buffin - Entité initiale - Cachet serveur | Recette mailSec    | true                       |                          | []                | []                                                                                           | false           |
